...
# Deteksi partisi SDCARD (bisa /dev/mmcblk1p1 atau /dev/mmcblk1)
SD_PART=""
if [ -b "${SD_DRIVE}p1" ]; then
    SD_PART="${SD_DRIVE}p1"
elif [ -b "${SD_DRIVE}1" ]; then
    SD_PART="${SD_DRIVE}1"
elif [ -b "$SD_DRIVE" ]; then
    SD_PART="$SD_DRIVE"
fi

if [ -n "$SD_PART" ] && blockdev --getsize64 "$SD_PART" > /dev/null 2>&1; then
    FSTYPE=$(blkid -o value -s TYPE "$SD_PART" 2>/dev/null)
    if [ "$FSTYPE" != "ext4" ]; then
        echo -e "${GREEN}Memformat $SD_PART ke ext4...${NC}"
        mkfs.ext4 -F "$SD_PART"
    else
    ...
    if ! grep -q "$SD_PART" /etc/fstab 2>/dev/null; then
        echo "$SD_PART /mnt/sdcard ext4 defaults,noatime 0 2" >> /etc/fstab
    fi
    mount -a
...
# Gunakan systemd-zram-generator (native di systemd Debian 13+)
apt install systemd-zram-generator -y 2>/dev/null || apt install zram-tools -y

if systemctl cat systemd-zram-generator > /dev/null 2>&1; then
    mkdir -p /etc/systemd/zram-generator.conf.d
    cat << 'EOF' > /etc/systemd/zram-generator.conf.d/50-zram.conf
[zram0]
zram-size = 512
...
apt install nginx mariadb-server php-fpm php-mysql php-cli php-curl php-gd php-mbstring php-xml php-zip procps iputils-ping -y
...
# Start PHP-FPM agar socket terdeteksi
PHP_FPM_SERVICE=$(systemctl list-unit-files 2>/dev/null | grep -oP 'php\S*fpm\.service' | head -1)
if [ -z "$PHP_FPM_SERVICE" ]; then
    PHP_FPM_SERVICE=$(ls /lib/systemd/system/php*-fpm.service 2>/dev/null | head -1 | xargs basename)
fi
...
if [ -z "$PHP_SOCK" ]; then
    echo -e "${RED}Error: PHP-FPM socket tidak ditemukan! Fallback ke TCP 127.0.0.1:9000.${NC}"
    PHP_SOCK="127.0.0.1:9000"
fi

# Tentukan format fastcgi_pass (unix socket vs TCP)
if [[ "$PHP_SOCK" == /* ]]; then
...
<div class="container">
    ...
    <a href="http://<?= $_SERVER['SERVER_ADDR'] ?? gethostbyname(gethostname()) ?>:8080" class="btn" target="_blank">Launch FileBrowser</a>
